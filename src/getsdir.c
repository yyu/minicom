...
static void free_up(void);	/* free-up used memory */
            ...
            mode_t modemask, GETSDIR_ENTRY **datptr, int *len)
{
  ...
  *len = 0;				/* longest name */
  ...
  for (cnt = 1; ; ++cnt) {        /* for buffer count 1 to whatever... */
    ...
    /* get a new buffer */
    if ((datb_cur = calloc(1, sizeof(DAT_BUF))) == NULL) {
      closedir(dirp);
      return -1;
    }
    ...
    if (datb_frst == NULL)		/* if first buff... */
    ...
    for (datb_cur->cnt = 0; datb_cur->cnt < MAX_CNT;)
      if ((dp = readdir(dirp))) {/* get next directory entry */
	if (!strcmp(dp->d_name, "."))
	  continue;
  ...
  if ((*datptr = (GETSDIR_ENTRY *)calloc(cnt, sizeof(datb_cur->data))) == NULL) {
...
{
  if ((datb_cur = datb_frst) != NULL)	/* start with first */
    do {                                /* and free 'em all */
      datb_frst = datb_cur->nxt;
      free(datb_cur);
